...
type DecryptScreen struct {
	width    int
	height   int
	api      DecryptScreenAPI
	inputs   []textinput.Model
	focus    int
	errorMsg string
}

func NewDecryptScreen(api DecryptScreenAPI) DecryptScreen {
	a := DecryptScreen{
		api:    api,
	...
	a.inputs[0].Placeholder = "Путь записи расшифрованного файла"
...
func (a DecryptScreen) Init() tea.Cmd {
	return textinput.Blink
}

func (a DecryptScreen) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmds []tea.Cmd
	...
	case tea.KeyMsg:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && a.focus == len(a.inputs)-1 {
				...
				if err := a.api.Decrypt(login, password, email); err != nil {
...
func (a DecryptScreen) View() string {
	title := "Дешифрование"
...
func (a *DecryptScreen) SetSize(width, height int) {
	a.width = width
	a.height = height
}
