...
type RegisterScreen struct {
	width    int
	height   int
	api      RegisterScreenAPI
	inputs   []textinput.Model
	focus    int
	errorMsg string
}

func NewRegisterScreen(api RegisterScreenAPI) RegisterScreen {
	a := RegisterScreen{
		api:    api,
...
func (a RegisterScreen) Init() tea.Cmd {
	return textinput.Blink
}

func (a RegisterScreen) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmds []tea.Cmd
	...
	case tea.KeyMsg:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && a.focus == len(a.inputs)-1 {
				...
				if err := a.api.Register(login, password, email); err != nil {
...
func (a RegisterScreen) View() string {
	title := "Регистрация"
...
func (a *RegisterScreen) SetSize(width, height int) {
	a.width = width
	a.height = height
}
